...
class Pose2VideoPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if attn_masked:
            attn_mask = rearrange(torch.tril(torch.ones((temporal_adaptive_step, temporal_adaptive_step))).unsqueeze(-1).repeat(1,1,temporal_window_size * temporal_window_size),"p q (u v) -> (p u) (q v)", u=temporal_window_size, v=temporal_window_size).unsqueeze(0).to(device)
        else:
            attn_mask = None
        ...
        if(temporal_kv_cache):
            self.denoising_unet.clear_kv_cache()
            self.denoising_unet.set_kv_cache(1, temporal_window_size)
        else:
            self.denoising_unet.clear_kv_cache()
            self.denoising_unet.set_kv_cache(0, temporal_window_size)
            ...
            for i in range(windows + temporal_adaptive_step - 1):
                ...
                for j in range(jump):
                    ...
                    noise_pred = self.denoising_unet(
                        ...
                        ut, 
                        encoder_hidden_states=encoder_hidden_states,
                        pose_cond_fea=pose_fea[:,:,l:r,:,:],
                        return_dict=False,
                        attention_mask = attn_mask[:,:r-l,:r-l] if attn_masked else attn_mask,
                        caching=(j==jump - 1) and (i>=temporal_adaptive_step)
                    )[0]
                    if do_classifier_free_guidance:
