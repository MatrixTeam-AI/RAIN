...
class UNet3DConditionModelExplicitReference(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.time_embedding = TimestepEmbedding(timestep_input_dim, time_embed_dim, cond_proj_dim=cond_proj_dim)
        if not(cond_proj_dim is None):
            torch.nn.init.zeros_(self.time_embedding.cond_proj.weight)
    ...
    def forward(
        ...
        if(not len(timesteps.shape) == 2):
            timesteps = timesteps.expand(sample.shape[0])
            t_emb = self.time_proj(timesteps)
            t_emb = t_emb.to(dtype=self.dtype)
            emb = self.time_embedding(t_emb, timestep_cond)
        else:
            fdim = timesteps.shape[1]
            t_emb = self.time_proj(rearrange(timesteps, "n f -> (n f)"))
            t_emb = t_emb.to(dtype=self.dtype)
            emb = rearrange(self.time_embedding(t_emb, timestep_cond), "(n f) c -> n c f", f=fdim)
