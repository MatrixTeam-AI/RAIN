...
class XFormersAttnProcessor:
    ...
    def __call__(
        ...
        key = attn.to_k(encoder_hidden_states, *args)
        value = attn.to_v(encoder_hidden_states, *args)
        if not (additional_k is None):
            key = torch.cat([key, additional_k], dim=1)
        if not (additional_v is None):
            value = torch.cat([value, additional_v], dim=1)
        ...
        hidden_states = hidden_states / attn.rescale_output_factor
        if return_kv:
            return hidden_states, key_copy, value_copy
        return hidden_states
