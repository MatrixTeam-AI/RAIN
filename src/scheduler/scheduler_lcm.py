...
class LCMScheduler(SchedulerMixin, ConfigMixin):
    ...
    def step(
        ...
        if(len(model_output.shape) == 5 and len(timestep.shape) == 2):
            alpha_prod_t = rearrange(alpha_prod_t[:, :, None, None, None], "b f c h w -> b c f h w")
            alpha_prod_t_prev = rearrange(alpha_prod_t_prev[:, :, None, None, None], "b f c h w -> b c f h w")
